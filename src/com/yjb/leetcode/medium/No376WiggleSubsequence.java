...
public class No376WiggleSubsequence {
    ...
    private static int solution1(int[] nums) {
        ...
        int[] up = new int[nums.length]; // up[i]表示到元素i为止，最后以上升结束的最长摆动子串长度
        int[] down = new int[nums.length]; // down[i]表示到元素i为止，最后以下降结束的最长摆动子串长度
        Arrays.fill(up, 1);
        Arrays.fill(down, 1);
     ...
     * https://www.cnblogs.com/grandyang/p/5697621.html
     * <p>
     * up和down更新的思路大致和solution1一样，只不过不使用数组而是直接用两个变量记录状态
     */
